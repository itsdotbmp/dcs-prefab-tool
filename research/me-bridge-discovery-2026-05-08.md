...
**Followup convention (2026-05-09):** The `dcs-sms me` CLI sidesteps the ME-table-vs-runtime-engine axis split by using **`--north / --east / --alt`** as the public flag triplet for placement verbs (instead of `--x / --y` or `--x / --z`). The runtime 3D engine uses `(x, y=altitude, z=E–W)` — opposite "y" semantics from the mission table — so any flag named `y` would mislead users of one of the two conventions. `north`/`east` is unambiguous regardless of background; the verb internally writes to mission-table `g.x = north`, `g.y = east`. The full rationale lives at the top of `tools/me-mod/lua/dcs_sms_me/verbs.lua` so it's discoverable from the code path that actually does the translation.
