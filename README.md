...
- `framework/` — in-DCS Lua framework. Modules: `sms`, `sms.log`, `sms.utils`, `sms.countries`, `sms.skill`, `sms.alt_type`, `sms.waypoint`, `sms.units`, `sms.statics`, `sms.targets`, `sms.designations`, `sms.group` (+ `sms.spawn` factories), `sms.unit`, `sms.area`, `sms.timer`, `sms.rule`, `sms.static`, `sms.events`, `sms.weapon`, `sms.task`, `sms.commands`, `sms.options`. See [`AGENTS.md`](AGENTS.md) for cross-cutting rules and conventions, or [`docs/api/`](docs/api/) for per-function detail.
