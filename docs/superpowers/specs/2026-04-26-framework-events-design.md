...
- **Subscriptions are independent — double-subscribing produces double-firing** (amendment 2026-04-27 follow-up). Each `connect` / `u:connect` / `g:connect` call returns its own Connection with its own state; the `g:connect(DEAD)` `fired_once` latch is per-connection, not per-group. Two `g:connect(DEAD, fn)` calls on the same group will both fire when the group dies (each once). Standard pub/sub: caller controls duplication, `conn:disconnect()` is the escape hatch.
- **Timer access goes through `sms.timer`** (amendment 2026-04-27 follow-up). The g:connect(DEAD) deferred check uses `sms.timer.after(0, ...)` rather than raw `timer.scheduleFunction`. `sms.unit.destroy(u, {emit_event=true})` uses a new tiny `sms.timer.now()` wrapper for the synthesized event's `time` field instead of raw `timer.getTime()`. events.lua now asserts `sms.timer` is loaded.
