...
func init() {
	registerMe("unit", "set-name", meUnitSetNameCmd)
}
...
func meUnitSetNameCmd(args []string, stdout, stderr io.Writer) int {
	...
	var (
		...
		flagNewName    = fs.String("new-name", "", "new unit name")
	...
	resp, exitCode := runMeVerb("unit_set_name", luaArgs, *flagTimeout, *flagSavedGames, stderr)
	if exitCode != 0 {
		return exitCode
	}
