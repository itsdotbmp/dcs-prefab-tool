# framework/AGENTS.md — framework reference for AI agents

This is the orientation document for AI agents (and humans) working in `framework/`. It exists so an agent dropped in cold can write idiomatic dcs-sms code without grepping every module first.
...
> **Working elsewhere?** See [`../AGENTS.md`](../AGENTS.md) for repo orientation and cross-cutting rules (commits, versioning, doc sync). For the host-side CLI see [`../tools/cmd/dcs-sms/AGENTS.md`](../tools/cmd/dcs-sms/AGENTS.md); for the ME-mod and the `me <noun> <verb>` verb namespace see [`../tools/me-mod/AGENTS.md`](../tools/me-mod/AGENTS.md).
>
> **Companion documents:**
...
## 2. Where the framework runs

`framework/*.lua` runs in the **mission environment**: sandboxed by `Scripts/MissionScripting.lua`. `os`, `io`, `lfs` are nilled. Lua 5.1. `print` is silent — use `env.info` / `env.error` (or `sms.log.*`).

The host-side CLI and the Mission Editor mod run in **different** Lua environments — see [`../AGENTS.md`](../AGENTS.md#2-three-lua-environments) for the matrix. Framework code must assume mission-environment constraints.
...
For one-shot (re)loading of the whole framework in a mission, use [`load_all.lua`](load_all.lua), which `dofile`s every module in the order above.
...
- **Update the §7 module index in this file** as part of the same PR. Adding or removing public surface without updating the index is a regression — agents and humans both lose visibility.
- **Update the per-module reference page at [`../docs/api/<module>.md`](../docs/api/)** in the same PR. Add or revise the entry for every public symbol you touch — signature, options table, return value, runnable example. A PR that ships new public surface without a corresponding `docs/api/` entry is incomplete. This rule is parallel to the §7 index rule and applies at every stage (spec → plan → implementation → review).
- If your change introduces a cross-module pattern worth showcasing, add or update a recipe in [`../docs/api/examples.md`](../docs/api/examples.md) too.
- Add or extend a smoke test under `test/` (`smoke_<module>.ps1` driven by `tools/dcs-sms.exe`; shared helpers live in `test/_smoke.psm1`).
